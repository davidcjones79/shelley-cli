...
func main() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(flag.CommandLine.Output(), "  coord-cli [flags] <cmd>       Manage coordinator (clear-tasks, scale, etc)\n")
		fmt.Fprintf(flag.CommandLine.Output(), "  watch [flags]                 Live CLI dashboard for coordinator\n")
...
func runCoord(global GlobalConfig, args []string) {
	...
	mux.HandleFunc("/api/task-start", coord.HandleTaskStart)
	mux.HandleFunc("/api/heartbeat", coord.HandleHeartbeat)
	mux.HandleFunc("/api/stuck-tasks", coord.HandleStuckTasks)
	mux.HandleFunc("/api/reset-stuck-tasks", coord.HandleResetStuckTasks)
